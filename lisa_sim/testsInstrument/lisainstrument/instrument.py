...
class Instrument:
    ...
    def testXgw(self):
        # t_electro_isi = lambda mosa, x: self.interpolate(
        #     x, -self.electro_delays_isis[mosa]
        # )
        #
        # t_delayed_distant_carrier_offsets = self.interpolate(
        #     carrier_offsets, -self.pprs[mosa]
        # )

        # 示例数据点
        x_values = np.arange(1, 60)
        y_values = np.arange(1, 60)

        # 要估算的点
        x = 2.5
        # 进行拉格朗日插值
        a = self.interpolate(y_values, -0.5/4)
        a = 10
    ...
    def write(self, output="measurements.h5", mode="w-", keep_all=False):
        ...
        with File(output, mode) as hdf5:
            ...
            if keep_all:
                ...
                self.scet_wrt_tps_distant.write(hdf5, "scet_wrt_tps_distant")
