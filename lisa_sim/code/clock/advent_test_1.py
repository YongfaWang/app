...
plt.loglog(f, np.sqrt(S_total), 'k-', label='Total Amplitude Spectral Density', linewidth=6)
plt.loglog(f, np.sqrt(S_wp), 'r--', label='White Phase Noise ASD', linewidth=6)
plt.loglog(f, np.sqrt(S_wf), 'g--', label='White Frequency Noise ASD', linewidth=6)
plt.loglog(f, np.sqrt(S_ff), 'b--', label='Flicker Frequency Noise ASD', linewidth=6)
plt.loglog(f, np.sqrt(S_rw), 'm--', label='Random Walk Noise ASD', linewidth=6)
plt.xlabel('Frequency (Hz)', fontsize=25)
plt.ylabel('ASD (Hz/√Hz)', fontsize=25)
...
# 绘制测量数据和拟合模型的对比图
plt.figure(figsize=(10, 6.18))
plt.loglog(tau_measured, sigma_measured, 'bo-', label='USO(True)', linewidth=6)
plt.loglog(tau_sim, sigma_sim, 'r--', label='Predicted(L-M algorithm)', linewidth=6)
plt.loglog(tau_sim, sigma_sim_old, 'g--', label='Predicted(Least squares)', linewidth=6)
plt.xlabel('Averaging Time $\\tau$ (s)', fontsize=25)
plt.ylabel('Allan Deviation', fontsize=25)
plt.grid(True, which='both', linestyle='--')
...
plt.loglog(tau_sim, sigma_full, 'k-', label='Full model', linewidth=6)
params_wp = np.array([params_opt[0], 0, 0, 0])
sigma_wp = allan_deviation(tau_sim, params_wp)
plt.loglog(tau_sim, sigma_wp, 'r--', label='White phase noise only', linewidth=6)
params_wf = np.array([0, params_opt[1], 0, 0])
sigma_wf = allan_deviation(tau_sim, params_wf)
plt.loglog(tau_sim, sigma_wf, 'g--', label='White frequency noise only', linewidth=6)
params_ff = np.array([0, 0, params_opt[2], 0])
sigma_ff = allan_deviation(tau_sim, params_ff)
plt.loglog(tau_sim, sigma_ff, 'b--', label='Flicker frequency noise only', linewidth=6)
params_rw = np.array([0, 0, 0, params_opt[3]])
sigma_rw = allan_deviation(tau_sim, params_rw)
plt.loglog(tau_sim, sigma_rw, 'm--', label='Random walk noise only', linewidth=6)
plt.xlabel('Averaging Time $\\tau$ (s)', fontsize=18)
plt.ylabel('Allan Deviation', fontsize=18)
