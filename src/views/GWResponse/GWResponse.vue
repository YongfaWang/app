...
export default {
  ...
  data() {
    return {
      ...
      showIniConfig: true,
      logData: "",
      xmlPath: JSON.parse(localStorage.getItem("appSettings")).homeDir + "/tests-gw-response/gw_response.xml",
      pyPath: JSON.parse(localStorage.getItem("appSettings")).homeDir + "/tests-gw-response/main_response.py",
    };
  },
  methods: {
    async copyLog() {
